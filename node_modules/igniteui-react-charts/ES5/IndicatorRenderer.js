...
import { Base, fromEnum, Number_$type, Point_$type, typeCast, toEnum, markType } from "igniteui-react-core/ES5/type";
 ...
 * @hidden
 */
var IndicatorRenderer = /** @class */ (function (_super) {
    ...
    IndicatorRenderer.f = function (a, b, c, d, e, f, g, h, i) {
        var j = typeCast(GeometryGroup.$, h.aj);
        var k = typeCast(GeometryGroup.$, i.aj);
        var l = c;
        var m = b;
        ...
        for (var p = 0; p < a; ++p) {
            var q = g.item(p);
            q.c = { $type: Point_$type, x: b(p), y: f };
            q.b = { $type: Point_$type, x: b(p), y: c(p) };
            var r = false;
            if (p > 0) {
                if (c(p) <= c(p - 1)) {
                    r = true;
