...
import { Type, typeCast, Number_$type, Point_$type, fromEnum, runOn, delegateRemove, delegateCombine, markType } from "igniteui-react-core/ES2015/type";
import { ValueBrushScale } from "./ValueBrushScale";
import { FastItemIntColumn } from "igniteui-react-core/ES2015/FastItemIntColumn";
...
export class ScatterContourSeries extends ScatterTriangulationSeries {
    constructor() {
        ...
        this.nh = new LinearContourValueResolver();
    }
    fs(a, b) {
        let c = this.gu(b);
    ...
    nr(a) {
        ...
        c = e.p0;
        b = e.p1;
        let f = new ScalerParams(1, b, c, this.lz.b8);
        f.c = this.getEffectiveViewport1(a);
        let g = new ScalerParams(1, b, c, this.l0.b8);
        g.c = this.getEffectiveViewport1(a);
        let h = this.lz;
        let i = this.l0;
        let j = this.l9.count;
        let k;
        let xArr_ = this.mc.asArray();
        k = (xArr_.slice(0));
        let l;
        let yArr_ = this.md.asArray();
        l = (yArr_.slice(0));
        let m = this.nl.asArray();
        let n = this.l9.asArray();
        let o = this.ma.asArray();
        let p = this.mb.asArray();
        let q = this.nh.j(this.nl);
    ...
    dw(a, b, c) {
        return super.dw(a, b, c) && this.nh != null && this.mc != null && this.md != null && this.l9 != null && this.ma != null && this.mb != null && this.mc.count > 0 && this.md.count > 0 && this.l9.count > 0 && this.ma.count > 0 && this.mb.count > 0;
    }
    ar(a, b) {
        let c = this.ni.b4(a, b);
        if (c != null) {
...
export class ScatterContourSeriesView extends SeriesView {
    ...
    bz(a, b) {
        ...
        for (let e = 0; e < b.length; e++) {
            ...
            if (d != null) {
                let m = d.getBrushByValue(f, this.bv.nl);
                if (m == null) {
                    m = this.e.k2;
                }
                this.bx.add(m);
    ...
    b4(a, b) {
        for (let c = 0; c < this.by.count; c++) {
            let d = this.by._inner[c];
            let e = typeCast(PathGeometry.$, d.aj);
            if (e != null) {
                for (let f = 0; f < e.b.count; f++) {
                    let g = e.b._inner[f];
                    let h = e.b._inner[f]._startPoint;
