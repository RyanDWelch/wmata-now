import { FinancialChartDropDownControlViewModel } from "./FinancialChartDropDownControlViewModel";
import { FinancialIndicatorTypeCollection } from "./FinancialIndicatorTypeCollection";
import { FinancialOverlayTypeCollection } from "./FinancialOverlayTypeCollection";
import { TrendLineType } from "igniteui-react-core/ES2015/TrendLineType";
import { FinancialChartVolumeType } from "./FinancialChartVolumeType";
import { Type } from "igniteui-react-core/ES2015/type";
/**
 * @hidden
 */
export declare class FinancialChartIndicatorMenuViewModel extends FinancialChartDropDownControlViewModel {
    static $t: Type;
    i: FinancialIndicatorTypeCollection;
    j: FinancialOverlayTypeCollection;
    k: TrendLineType;
    h: FinancialChartVolumeType;
}
