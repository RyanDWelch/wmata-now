import { Base, IList$1, IEnumerator, IEnumerator$1, Type } from "igniteui-react-core/ES2015/type";
/**
 * @hidden
 */
export declare class SortedListView$1<T> extends Base implements IList$1<T> {
    static $t: Type;
    protected $t: Type;
    private c;
    private d;
    constructor($t: Type, a: IList$1<T>, b: IList$1<number>);
    add(a: T): void;
    clear(): void;
    contains(a: T): boolean;
    indexOf(a: T): number;
    insert(a: number, b: T): void;
    readonly a: boolean;
    readonly isReadOnly: boolean;
    remove(a: T): boolean;
    removeAt(a: number): void;
    item(a: number, b?: T): T;
    readonly count: number;
    readonly b: boolean;
    readonly e: any;
    private _getEnumeratorObject;
    getEnumeratorObject(): IEnumerator;
    private _getEnumerator;
    getEnumerator(): IEnumerator$1<T>;
    copyTo(a: T[], b: number): void;
}
