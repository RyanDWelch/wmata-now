...
import { Type, typeCast, fromEnum, runOn, markType } from "igniteui-react-core/ES2015/type";
 ...
 * @hidden
 */
export class BarSeries extends VerticalAnchoredCategorySeries {
    constructor() {
        super();
        this.ab = BarSeries.$;
        this.na = new BarFramePreparer(1, this, this.p8, this, this, this.p8.cn);
    }
    bf() {
    ...
    lt(a) {
        if (this.xAxis == null || this.yAxis == null || this.m6.f.count == 0) {
            return Rect.empty;
        }
        let b = this.bi.bs;
        let c = this.bi.br;
        ...
        let i = truncate(Math.floor(g / this.n6(this.bi)));
        let j = truncate(Math.floor(h / this.n6(this.bi)));
        let k = i - this.n7(this.bi);
        let l = j - this.n7(this.bi);
        if (k < 0) {
            k = 0;
        }
        ...
        if (l > this.m6.f.count - 1) {
            l = this.m6.f.count - 1;
        }
        let m = this.m6.f._inner[k];
        let n = this.m6.f._inner[l];
        let o = Math.abs(f.y - m[0]);
        let p = Math.abs(f.y - n[0]);
        let q = this.o6(this.ne);
        let r = this.yAxis.jq(b, c, d);
        if (o < p) {
            let s = m[0] - 0.5 * r;
            let t = m[1];
            let u = q;
    ...
    o6(a) {
        ...
        if (!c.isEmpty && !d.isEmpty && this.xAxis != null) {
            let f = new ScalerParams(0, c, d, this.xAxis.b8, e);
            b = this.xAxis.dv(this.xAxis.kd, f);
        }
        return b;
    }
    an(a) {
    ...
    c9(a) {
        ...
        let g = new ScalerParams(0, d, d, this.xAxis.b8, f);
        let h = new ScalerParams(0, d, d, this.yAxis.b8, f);
        let i = this.yAxis != null ? this.yAxis.dv(e, h) : NaN;
        let j = this.yAxis != null ? this.na.r(this.yAxis, d, d, f) : 0;
        i += j;
        let k = this.xAxis != null && this.valueColumn != null && e < this.valueColumn.count ? this.xAxis.dv(this.valueColumn.item(e), g) : NaN;
        if (!isNaN_(k)) {
    ...
    on(a, b) {
        ...
        let h = this.yAxis;
        let i = new ScalerParams(0, d, e, this.yAxis.b8, f);
        let j = this.xAxis;
        let k = j.dv(j.kd, g);
        let l = this.yAxis.jq(d, e, f);
        let m = typeCast(BarSeriesView.$, b);
        if (isNaN_(l) || isInfinity(l) || isNaN_(k)) {
            m.c7.count = 0;
            return;
        }
        this.nd.x(this, this.ny(), this.yAxis, runOn(this, this.mz), this.n6(b), this.n7(b));
        this.nd.p = this.p9;
        this.nd.q = this.qa;
        this.nd.j = this.p9;
        this.nd.k = this.qa;
        ...
        for (let r = 0; r < c.count; ++r) {
            ...
            if (n) {
                this.oj(c, r, q, h, i, b.n);
            }
            this.nd.ae(v, false, false, false, false);
            v.al = this.nd.j;
            v.am = this.nd.k;
    ...
    hy(a, b, c, d) {
        ...
        switch (a) {
            ...
            case 0:
                ...
                break;
            case 2:
                if (this.pc != null && this.ox.cn.d > 0) {
                    this.jc(true);
                }
                break;
            case 3:
...
BarSeries.$t = markType(BarSeries, 'BarSeries', VerticalAnchoredCategorySeries.$, [IIsCategoryBased_$type, IBarSeries_$type]);
Type.dep(DependencyProperty, PropertyMetadata, BarSeries, 'raisePropertyChanged', ['RadiusX:p9:qd', [1, 2], 'RadiusY:qa:qe', [1, 2]]);
/**
 * @hidden
 */
export class BarBucketCalculator extends CategoryBucketCalculator {
    ...
    g(a) {
        ...
        let h = new ScalerParams(0, b, c, e.yAxis.b8, g);
        let i = Math.floor(e.yAxis.dz(f.top, h, 0));
        let j = Math.ceil(e.yAxis.dz(f.bottom, h, 0));
        if (!e.yAxis.b8) {
            j = Math.ceil(e.yAxis.dz(f.top, h, 0));
            i = Math.floor(e.yAxis.dz(f.bottom, h, 0));
        }
